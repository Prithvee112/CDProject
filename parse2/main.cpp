...
void calculate_follow(map<char,vector<vector<char>>> &prod) {
    map<char, std::set<char>> follow;
                ...
                for (size_t i = 0; i < production.size(); ++i) {
                    ...
                    if (symbol >= 'A' && symbol <= 'Z') {
                        if (i == production.size() - 1) {
                            auto& follow_lhs = follow[lhs];
                            auto& follow_symbol = follow[symbol];
                            size_t size_before = follow_symbol.size();

                            follow_symbol.insert(follow_lhs.begin(), follow_lhs.end());

                            if (size_before != follow_symbol.size()) {
                                changed = true;
                            }
                        } else {
                            ...
                            if (follow_symbol.insert(next_symbol).second) {
                                changed = true;
                            }
                        }
                    }
...
void calculate_first(map<char,set<char>> &fmp){
    for(auto q : fmp){
        string ans ="";
        ans += q.first;
        ans += " = {";
...
int main(){
    int i,j;
    ifstream fin("C:\\Users\\Pranitha\\Desktop\\3rd year\\CD\\production.txt");
    string num;
    vector<int> fs;
    vector<vector<int>> a;
    char start;
    ...
    while(getline(fin,num)){
        ...
        cout<<num.substr(0,1)+" -> "+num.substr(4,num.size()-3)<<'\n';
        vector<char> temp;
        char s = num[0];
        for(i=4;i<num.size();i++){
            if(num[i] == '|'){
            mp[s].push_back(temp);
            temp.clear();
            }
